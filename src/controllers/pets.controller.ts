...
export const updatePet = asyncHandler(async (req: AuthRequest, res) => {
  const pet = await petsService.updatePet(req.params.id, req.userId!, req.body);
  sendSuccess(res, 200, 'Pet updated successfully', pet);
});

export const deletePet = asyncHandler(async (req: AuthRequest, res) => {
  const pet = await petsService.deletePet(req.params.id, req.userId!);
